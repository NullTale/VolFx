            ...
            using (new EditorGUI.DisabledGroupScope(!enabledProperty.boolValue))
            {
                ...
                if (valueProperty.propertyType == SerializedPropertyType.LayerMask)
                    valueProperty.intValue =
                        InternalEditorUtility.ConcatenatedLayersMaskToLayerMask(EditorGUI.MaskField(position, label, InternalEditorUtility.LayerMaskToConcatenatedLayersMask(valueProperty.intValue),

        ...
        private ReorderableList _getList(SerializedProperty property)
        {
            if (m_List == null)
            {
                ...
                void _createElementOfType(object type)
                {
                    ...
                    var varName = _unique(_format(element.GetType().Name));
                    ...
                    // -----------------------------------------------------------------------
                    string _unique(string nameIn)
                    {
                        var letter = 0;
                        
                        while (true)
                        {
                            var finalName = nameIn + $" {_letter(letter ++)}";
                            if (_contains(finalName) == false)
                                return finalName;
                        }
                        ...
                        bool _contains(string objName)
                        {
                            for (var n = 0; n < propertyList.arraySize; n++)
                            {
                                var e = propertyList.GetArrayElementAtIndex(n);
                                if (e.objectReferenceValue == null)
                                    continue;
                                
                                if (e.objectReferenceValue.name == objName)
                                    return true;
                            }
